...
function Update-NuSpecFile
{
	...
	{		
		...
		if ($NoPromptForReleaseNotes)
		{
			...
			if ($ReleaseNotes -eq $null -or $ReleaseNotes.Length -eq 0) { $ReleaseNotes = "See Project Site" }
		}
...
try
{
    ...
    if (String-IsNullOrWhitespace $PackageFilePath)
    {
	    ...
	    $packCommand = "& ""$NuGetExecutableFilePath"" pack ""$fileToPack"" $PackOptions -Verbosity detailed"
