...
#==========================================================
param ([string]$ProjectFilePath, [string]$OutputDirectory, [string]$Configuration, [string]$Platform, [string]$PackageDirectory)
...
if ($PackageDirectory -eq $null -or $PackageDirectory.Length -eq 0) { $PackageDirectory = $OutputDirectory }
if (!(Test-Path $PackageDirectory)) { New-Item -itemtype directory $PackageDirectory }
...
& "$THIS_SCRIPTS_DIRECTORY\New-NuGetPackage.ps1" -ProjectFilePath "$ProjectFilePath" -VersionNumber $versionNumber -ReleaseNotes $releaseNotes -PackOptions "-OutputDirectory ""$PackageDirectory"" -Properties $packProperties -NonInteractive $additionalPackOptions" -DoNotUpdateNuSpecFile -NoPrompt -Verbose